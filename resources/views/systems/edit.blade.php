...
<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li>
			<a href="{{ route('systems.index') }}">{{ $pageTitle }}</a>
		</li>
...
<section class="content">
	...
	<div class="box">
		<form class="form-horizontal" action="{{ route('systems.update', $system->id) }}" method="POST">
			@csrf @method('PUT')
			<div class="box-body">
                <div class="form-group">
                    <label class="col-sm-2 control-label" for="order">Class Icon</label>
                    <div class="col-sm-8">
                        <input type="text" id="icon" name="icon" class="form-control" value="{{ $system->icon }}">
                    </div>
                </div>
				<div class="form-group">
					<label class="col-sm-2 control-label" for="name">Tiêu đề </label>
					<div class="col-sm-8">
						<input autocomplete="off" name="title" type="text" class="form-control" value="{{ $system->title }}" placeholder="Nhập tiêu đề">
					</div>
				</div>
				{{--<div class="form-group">
					...
					<div class="col-sm-8">
						<div class="input-group">
							...
							<input readonly class="form-control cursor browseServer" type="text" name="icon" value="{{ $systems->icon ? url('upload/shares/'.$system->icon) : '' }}">
						</div>
						<div class="blah-wrap" {!! $system->icon ? 'style="display: block"' : '' !!}>
							<img class="blah-img" src="{{ url('upload/shares/'.$systems->icon) }}">
						</div>
					</div>
				</div>--}}
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						<textarea id="editor" name="description" class="form-control">{{ $system->description }}</textarea>
					</div>
				</div>
                <div class="form-group">
                    <label class="col-sm-2 control-label" for="order">Order</label>
                    <div class="col-sm-8">
                        <input type="number" id="order" name="order" class="form-control" value="{{ $system->order }}">
                    </div>
                </div>
				<!-- /.box-body -->
				<div class="form-group">
					<div class="col-sm-offset-2 col-sm-8">
						...
						<a href="{{ route('systems.index') }}" class="btn btn-default pull-right">« Quay lại</a>
					</div>
				</div>
			</div>
		</form>
